...

{
	...
	{
		private ["_type", "_relPos", "_azimuth", "_fuel", "_damage", "_orientation", "_varName", "_init", "_simulation", "_ASL", "_newObj", "_placingPoint", "_scale"];
		_type = _x select 0;
		_relPos = _x select 1;
		...
		//added support for placingPoint info for simpleObjects
		if ((count _x) > 9) then {_placingPoint = _x select 9;};
		if ((count _x) > 10) then {_scale = _x select 10;};
		if ((count _x) > 11) then {_ASL = _x select 11};
		...
		//will now take source object into account when creating objects. Allows easier manipulation.
		_newPos = [_posX + (_newRelPos select 0), _posY + (_newRelPos select 1), _z + ((_this select 0) select 2)];
		//_newPos = [_posX + (_newRelPos select 0), _posY + (_newRelPos select 1), _z + ((_this select 0) select 2)];
		//new objects are created a bit differently.
		_newObj = "";
		//Create the object and make sure it's in the correct location
		//objects with \/ contain a path to the model file, and need to be treated as simpleObjects.
		if ((_type find "\") > -1 || (_type find "/") > -1) then 
		{
			_newObj = createSimpleObject [_type, [_newPos select 0, _newPos select 1, (_newPos select 2)], false];
			//_newObj = create3DENEntity ["Object", "Land_VR_CoverObject_01_kneel_F", _newPos, true];
			//if (!_ASL) then {_newObj setPos [_newPos select 0, _newPos select 1, _placingPoint select 2];} else {_newObj setPosASL [_newPos select 0, _newPos select 1, _placingPoint select 2]; _newObj setVariable ["BIS_DynO_ASL", true];};
		...
		} else 
		{
			...
			_newObj setDir (_azi + _azimuth);
			if (!_ASL) then {_newObj setPos [_newPos select 0, _newPos select 1, 0];} else {_newObj setPosASL _newPos; _newObj setVariable ["BIS_DynO_ASL", true];};
			//_newObj = create3DENEntity ["Object", _type, _newPos, true];
		};
		...
		if (!isNil "_scale") then {_newObj setObjectScale _scale;};
